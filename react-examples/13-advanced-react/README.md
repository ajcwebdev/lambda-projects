...
Create a class that extends React.Component (or leave off React if Component is imported). In the constructor method call super and set your state. In componentDidMount fetch needed data. In the render function return any child components that need to rendered.

2. Describe the different phases of the component lifecycle.

A component goes through birth, life, and death during its lifecycle phases. The lifecycle involves mounting and unmounting on the DOM. Birth is componentWillMount which happens before the component is mounted to the DOM. Life is componentDidMount which happens after the component is mounted. Death is componentWillUnmount which happens once the component is ready to be removed from the DOM.

3. Demonstrate an understanding of class component lifecycle methods.

componentWillMount: Component is about to appear on the screen from the render function. Since the component has not mounted yet there is no interaction with the DOM.

componentDidMount: Component is mounted and ready to receive data from external APIs. Event listeners are added and AJAX calls are started in this phase.

componentWillUnmount: Component is about to be removed from the DOM. Network requests are cancelled and event listeners are removed.

4. Define stateful logic.

Stateful logic is conditional logic that changes our component's state for example the state of being in dark mode versus not being in dark mode. To set dark mode conditional logic is used (if-then statements) and the conditional logic will affect the state of the component.

5. Describe how to test a React component with React Testing Library.

First test that the component render's without errors in a test function. Then test the functionality of the component by selecting specific parts of the component with getByLabelText, getByRole, or other methods from the testing library. fireEvent.change is used to enter dummy data into the selected parts of the component.
